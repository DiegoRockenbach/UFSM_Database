...
public class FileReadPerformance {
    ...
    public static void main(String[] args) throws IOException {
        ...
        measureReadTime("Leitura em blocos de 32 KB", () -> {
          try {
              readInBlocks(FILE_NAME, 32 * KB);
          } catch (IOException ex) {
              Logger.getLogger(FileReadPerformance.class.getName()).log(Level.SEVERE, null, ex);
          }
        });

        measureReadTime("Leitura em blocos de 128 KB", () -> {
          try {
              readInBlocks(FILE_NAME, 128 * KB);
          } catch (IOException ex) {
              Logger.getLogger(FileReadPerformance.class.getName()).log(Level.SEVERE, null, ex);
          }
        });

        measureReadTime("Leitura em blocos de 512 KB", () -> {
          try {
              readInBlocks(FILE_NAME, 512 * KB);
          } catch (IOException ex) {
              Logger.getLogger(FileReadPerformance.class.getName()).log(Level.SEVERE, null, ex);
          }
        });

        measureReadTime("Leitura em blocos de 1024 KB", () -> {
          try {
              readInBlocks(FILE_NAME, 1024 * KB);
          } catch (IOException ex) {
              Logger.getLogger(FileReadPerformance.class.getName()).log(Level.SEVERE, null, ex);
          }
        });
