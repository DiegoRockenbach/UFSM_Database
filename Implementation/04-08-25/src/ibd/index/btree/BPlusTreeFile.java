...
//        }
    ...
    @Override
    public ArrayList<Value> searchLarger(Key lowerBound) {
        ...
        // Find leaf node that holds the dictionary key
        LeafNode ln = (getRootID() == -1) ? (LeafNode) getNode(getFirstLeafID()) : findLeafNode(lowerBound);

        // Perform binary search to find index of key within dictionary
        DictionaryPair[] dps = ln.dictionary;
        int index = Utils.binarySearch(dps, ln.numPairs, lowerBound, this);

        // a negative index may simply mean that the key has less levels than the indexed keys.
        ...
        //add all values from the found index on to the resulting list
        ...
        // all found values are added to the resulting list
    ...
    @Override
    public ArrayList<Value> searchSmaller(Key upperBound) {
        ...
        LeafNode ln = getFirstPage();
        
        DictionaryPair[] dps = ln.dictionary;
        int index = 0;
        //traverses the node from the first index
        for (int i = index; i < dps.length; i++) {
            if (dps[i] != null) {
                //keeps adding the found values to the resulting list until an unfitting value is found
                if ((upperBound.compareTo(dps[i].key))<0) {
                    return values;
                }
                values.add(dps[i].value);
        ...
        while (currNode != null) {
            ...
            for (DictionaryPair dp : dps_) {
                ...
                //keeps adding the found values to the resulting list until an unfitting value is found
                if ((upperBound.compareTo(dp.key))<0) {
                    return values;
                }
                values.add(dp.value);
            }
    ...
    public ArrayList<Value> partialSearch(Key key) {
        ...
        //traverse the dictionary entries of the leaf node
        for (int i = index; i < dps.length; i++) {
            if (dps[i] != null) {
                //keeps adding the found values to the resulting list until a higher value is found
